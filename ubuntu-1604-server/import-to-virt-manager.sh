...
if [[ "$vm_name" == '' ]]; then
  ...
  until [[ "$vm_name" =~ ^([a-zA-Z0-9._-]+){1,}$ ]]; do
    read -rp "[Enter a VM name]: " -e -i 'ubuntu16.04-server' vm_name
  done
fi
...
# The build image file name should always be "ubuntu-1604-server", we change this when it's imported into virt-manager using $vm_name here
echo "[*]Copying VM files to virt-manager path..."
sudo cp build/ubuntu-1604-server "${dest_path}"/images/"${vm_name}".qcow2
