...
if [[ "$vm_name" == '' ]]; then
  ...
  until [[ "$vm_name" =~ ^([a-zA-Z0-9._-]+){1,}$ ]]; do
    read -rp "[Enter a VM name]: " -e -i "${vm_os}" vm_name
  done
fi
...
# The build image file name should always be "win-server", we change this when it's imported into virt-manager using $vm_name here
echo "[*]Copying VM files to virt-manager path..."
sudo cp build/"${vm_name}" "${dest_path}"/images/"${vm_name}".qcow2
...
virt-install \
  ...
  --boot loader=/usr/share/OVMF/OVMF_CODE_4M.secboot.fd,loader.readonly=yes,loader_secure=yes,loader.type=pflash,nvram="${dest_path}"/qemu/nvram/"${vm_name}"_VARS.fd \
  --tpm default \
